...
const Login = () => {
  ...
  const submitLogin = async (e) => {
    ...
    await axios
      .post('/user/token-login', JSON.stringify(`grant_type=&username=${email}&password=${password}&scope=&client_id=&client_secret=`), {
      ...
      .then((result) => {
        ...
        const signInResult = signIn({
          token: result?.data?.details?.token?.access_token,
          expiresIn: result?.data?.details?.token?.access_token_expires_in,
          tokenType: result?.data?.details?.token?.token_type,
          refreshToken: result?.data?.details?.token?.refresh_token,
          refreshTokenExpireIn: result?.data?.details?.token?.refresh_expires_in,
          authState: {
            email: email, 
            token: result?.data?.details?.token?.access_token, 
            refreshToken: result?.data?.details?.token?.refresh_token 
